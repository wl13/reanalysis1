#!/usr/bin/perl -w
#
#  MKtest.pm -- **McDonald-Kreitman Test (McDonald & Kreitman 1991)**
#    Original Script From: http://www.dpgp.org/aholloway/Software.html
...
sub calcMK
{
    ...
    my $POS       = 0;       
    my $numNT     = length $SEQ->[0]; # The length of the the first sequence
                                      # in ingroup species
    my $numSEQS   = scalar @{$SEQ};   # How many sequences in ingroup species
    my $numcodons = 0;
    my $Nfix      = 0;
    my $Sfix      = 0;
    my $numIN     = 0;
            ...
            for( my $x=0; $x<$numSEQS; $x++ )
            {
                ...
                if ( $AAX ne 'X' ) {
                    ...
                    if ( $tempcodon eq $OUTcodon1 ) {
                        ...
                        $difffromOUT++;
                        my $a=0;
                        while ( ($a < scalar @codons) && ($flag != 2) )
                        { ##only push each codon type on once
                ...
                if ( $difffromOUT ) {
                    if ( $INsame == $ins-1 ) {	## fixed diff from mel
                        ...
                        if ( $sameasOUT ) {	## 2 sim codons, 1 sameasOUT, 1 diff, calc path for diff add to poly
                            my $tempNdiff=3;
                            my $tempSdiff=0;
                            for ( my $y=0; $y < scalar @codons; $y++ )
                            {	## takes simple route of evol from mel to a sim that minimizes NS subs
    ...
    if ( $Npoly || $Spoly || $Nfix || $Sfix ) {		     
        ...
        my $FET_pvalue = calculateStatistic (
                                              n11=>$Npoly,
                                              n1p=>$Npoly+$Spoly,
                                              np1=>$Npoly+$Nfix,
	...
	return [$Npoly, $Nfix, $Spoly, $Sfix, $numcodons, $avgsamplesize, $FET_pvalue];
    }
    else {
	return -1;
    }
...
sub codon2codon { #join the codon1 and codon2 as a codonpair to get the pair value such as (0, 0) etc.
    my ( $codonpair ) = @_;
	...
	print STDERR "Bad codon pair \"$codonpair\"!!\n";
    	exit;
    }
}
...
e.g. for polarized analysis
	...
	perl MK_Test -outfile out.txt -pol pol -outgroup mel -outgroup yak -ingroup sim -dir ./your_fasta_directory

e.g. for unpolarized anlaysis

	perl MK_Test -outfile out.txt -pol unpol -outgroup mel -ingroup sim -dir ./your_fasta_directory
